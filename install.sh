...
# Download scripts
wget https://github.com/siamak136/PPPwn_WRT/raw/main/run.sh
if [ $? -ne 0 ]; then
    echo "Failed to download run.sh"
    exit 1
fi

wget https://github.com/siamak136/PPPwn_WRT/raw/main/kill.sh
if [ $? -ne 0 ]; then
...
if echo "$machine_arch" | grep -q "arch64"; then
    wget https://github.com/siamak136/PPPwn_WRT/raw/main/pppwn_arch64
    if [ $? -ne 0 ]; then
    ...
    wget https://github.com/siamak136/PPPwn_WRT/raw/main/pppwn_armv7
    if [ $? -ne 0 ]; then
    ...
    wget https://github.com/siamak136/PPPwn_WRT/raw/main/pppwn_x86_64
    if [ $? -ne 0 ]; then
    ...
    if [ "$BYTE_ORDER" == "Big Endian" ]; then
        wget https://github.com/siamak136/PPPwn_WRT/raw/main/pppwn_mips
        if [ $? -ne 0 ]; then
        ...
        wget https://github.com/siamak136/PPPwn_WRT/raw/main/pppwn_mipsel
        if [ $? -ne 0 ]; then
...
if [ "$firmware" = "11.00" ] || [ "$firmware" = "10.00" ] || [ "$firmware" = "9.00" ] || [ "$firmware" = "9.60" ]; then
    ...
    wget https://github.com/siamak136/PPPwn_WRT/raw/main/stage1_${firmware//.}.bin
    if [ $? -ne 0 ]; then
        echo "Failed to download stage1_${firmware//.}.bin"
        exit 1
    fi
    wget https://github.com/siamak136/PPPwn_WRT/raw/main/stage2_${firmware//.}.bin
    if [ $? -ne 0 ]; then
